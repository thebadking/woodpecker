...
func (c *Forgejo) dirRecursive(ctx context.Context, u *model.User, r *model.Repo, b *model.Pipeline, path string, maxDepth, currentDepth int, client *forgejo.Client) ([]*forge_types.FileMeta, error) {
	log.Info().Str("repo", r.FullName).Str("path", path).Str("commit", b.Commit).Int("depth", currentDepth).Int("maxDepth", maxDepth).Msg("Forgejo dirRecursive() called")
	var configs []*forge_types.FileMeta

	// List files in repository
	log.Debug().Str("repo", r.FullName).Str("owner", r.Owner).Str("name", r.Name).Str("commit", b.Commit).Str("path", path).Msg("Calling Forgejo ListContents API")
	contents, resp, err := client.ListContents(r.Owner, r.Name, b.Commit, path)
	if err != nil {
		if resp != nil && resp.StatusCode == http.StatusNotFound {
			log.Warn().Str("repo", r.FullName).Str("path", path).Int("status", resp.StatusCode).Msg("Forgejo ListContents returned 404 - path not found")
			return nil, errors.Join(err, &forge_types.ErrConfigNotFound{Configs: []string{path}})
		}
		log.Error().Str("repo", r.FullName).Str("path", path).Err(err).Msg("Forgejo ListContents failed with error")
		return nil, err
	}

	// Log what we found for debugging
	log.Debug().Str("repo", r.FullName).Str("path", path).Msgf("Forgejo ListContents returned %d items", len(contents))
	var allItems []string
